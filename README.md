...
- `goto:` 规则演示：兜底规则可重定向到始终可匹配的通用探索规则。
...
模板还支持 `goto:rule_name`。当前兜底规则有时会返回 `goto:explore`；引擎不会把它显示给用户，而是让同一条输入改由 `explore` 规则处理。`explore` 的模式是 `(.*)`，因此任何非空输入都能匹配，并得到“哪一部分最值得继续讨论？”一类回复。

`explore` 的优先级特意设为 `-1`，低于 `default` 的 `0`。这避免它在正常规则遍历时抢先匹配所有输入；它只会在 `goto:explore` 明确重定向时运行。这是对经典 ELIZA “重定向/分解-重组”思想的一个小而可靠的实现。
